...
post '/orders/:user_id' do
  begin
    order = Order.where(idempotency_key: params['idempotency_key']).first

    return {id: order.id, product_id: order.product_id, hour: order.hour, status: order.status}.to_json if order.present?

    user_id = params['user_id'].to_i
    order = Order.new
    order.user_id = user_id
    order.product_id = params[:product_id].to_i
    order.idempotency_key = params['idempotency_key']
